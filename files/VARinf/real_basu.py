...
def one_step_forecast(t,y_normed,args):
    ...
    y = y_normed[:,:t]
    y_forecast,residual = BasuEstimate(y,args.VARpen,args.VARMApen)
    np.save(f'result/{args.exp_name}/Insample_resid_basu_{args.VARpen}_{args.VARMApen}_{t}.npy',residual)
    end_time = time.time()
    residual_centered = residual - np.mean(residual, axis=0)
    cov_matrix = np.dot(residual_centered.T, residual_centered) / (t - 1)
    return y_forecast,cov_matrix,end_time-start_time
...
def main():
    ...
    for t in range(start_ind,T):
        ...
        time[t-start_ind] = results[t-start_ind][2]
